...
public function ventasfechaarticulo($fecha_inicio,$fecha_fin,$idcliente){
	$sql="SELECT DATE(v.fecha_hora) as fecha, u.nombre as usuario, p.nombre as cliente, a.nombre as articulo, d.cantidad as detalle_venta, v.total_venta, v.forma_pago, a.stock as venta
	    FROM venta v, articulo a, detalle_venta d 
	    INNER JOIN persona p ON v.idcliente=p.idpersona INNER JOIN usuario u ON v.idusuario=u.idusuario INNER JOIN articulo a ON v.idcliente=a.idarticulo INNER JOIN detalle_venta d ON v.idcliente=d.iddetalle_venta WHERE DATE(v.fecha_hora)>='$fecha_inicio' AND DATE(v.fecha_hora)<='$fecha_fin' AND v.idcliente='$idcliente'";
	return ejecutarConsulta($sql);
}
